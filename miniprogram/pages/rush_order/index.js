Page({
    ...
    getClassDetail() {
        ...
        db.collection('rush_class')
            .doc(that.data.class_id)
            .get({
                success(res) {
                    that.setData({
                        class_detail: res.data
                    })
                },
                fail(err) {
                    console.log(err);
    ...
    goOrder() {
        let that = this
        let name = that.data.class_detail.send_name
        let send_id = that.data.class_detail.send_id
        let order_id = that.data.class_detail._id
        if (!that.data.userInfo) {
            ...
            wx.showModal({
                ...
                success(res) {
                    if (res.confirm) {
                        let class_order = that.data.class_detail;
                        // 下单成功，添加到数据库中
                        let db = wx.cloud.database()
                        db.collection('Order_record')
                            .add({
                                data: {
                                    order_name: class_order.desc,
                                    send_user: class_order.send_name,
                                    price: class_order.price,
                                    get_send: that.data.userInfo.username,
                                    send_id: class_order.send_id,
                                    get_id: that.data.userInfo._id,
                                    type: class_order.type,
                                    id:order_id,
                                    is_finish:false,
                                ...
                                let class_only_id = that.data.class_id
                                db.collection('rush_class').doc(class_only_id).update({
                                    data: {
                                        status: true
                                    }
                                })
